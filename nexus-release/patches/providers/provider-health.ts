...
// are marked healthy and shown. This mirrors p-stream's `useProviderScrape`
...
function probeUrlFor(id: string): string | null {
  ...
  const encdec = "https://enc-dec.app";
  const hf =
    (import.meta.env.VITE_TMDB_EMBED_API_URL as string | undefined)?.replace(/\/$/, "") ??
    "https://zetic7z-tmdb-embed-api.hf.space";
  ...
  switch (id) {
    ...
    case "nexus-tmdb":
      return `${hf}/`;
    case "nexus-vidlink":
      return `${encdec}/api/enc-vidlink?text=1`;
    case "nexus-videasy":
      return `${encdec}/`;
    case "nexus-vidfast":
      return `${encdec}/api/enc-vidfast?text=test`;
    case "nexus-hexa":
      return `${encdec}/api/enc-hexa`;
    case "nexus-yflix":
      return `${encdec}/db/`;
    default:
      return null;
