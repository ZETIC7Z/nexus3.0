import { FullScraperEvents, RunOutput, ScrapeMedia } from "@nexus/providers";
...
function useBaseScrape() {
  ...
  const activeAllowedIdsRef = useRef<string[]>([]);

  const setAllowedIds = useCallback((ids: string[]) => {
    activeAllowedIdsRef.current = ids;
  }, []);

  const initEvent = useCallback((evt: ScraperEvent<"init">) => {
    const allowed = activeAllowedIdsRef.current;
    const validSourceIds =
      allowed.length > 0
        ? evt.sourceIds.filter((id) => allowed.includes(id))
        : evt.sourceIds;
    ...
    setSources(
      validSourceIds
        ...
        .filter((v): v is ScrapingSegment => v !== null)
        .reduce<Record<string, ScrapingSegment>>((a, v) => {
          a[v.id] = v;
          return a;
        }, {}),
    );
    setSourceOrder(validSourceIds.map((v) => ({ id: v, children: [] })));
  }, []);
...
export function useScrape() {
  ...
  const startScraping = useCallback(
    async (media: ScrapeMedia, startFromSourceId?: string) => {
      const isAnime = await checkIsAnime(media);
      const allowedIds = getAllowedSourceIds(media.type, isAnime);
      setAllowedIds(allowedIds);
      const providerInstance = getProviders();
      const allSources = providerInstance
        .listSources()
      ...
      const activeFiltered = filteredSourceOrder.filter(
        (id) => !failedSources.includes(id)
      );
      if (activeFiltered.length > 0) {
        filteredSourceOrder = activeFiltered;
      }
