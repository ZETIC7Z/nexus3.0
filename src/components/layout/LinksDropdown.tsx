    ...
    <div className="relative is-dropdown">
      ...
      <Transition animation="slide-down" show={open}>
        <div className="rounded-xl absolute w-64 bg-dropdown-altBackground top-full mt-3 right-0">
          <div className="relative overflow-hidden w-full h-[56px]">
            <Transition animation="slide-full-left" show={!deviceName} className="absolute inset-0 w-full">
              <DropdownLink
                onClick={() => {
                  setOpen(false);
                  openAuthModal("login");
            ...
            <Transition animation="slide-full-right" show={!!deviceName && !!bufferSeed} className="absolute inset-0 w-full">
              <DropdownLink className="text-white" href="/settings">
                ...
                <span className="ml-1">{nickname || "NEXUS"}</span>
              </DropdownLink>
            </Transition>
          </div>
          <Divider />
              ...
              <DropdownLink
                onClick={() =>
                  window.dispatchEvent(
                    new CustomEvent("nexus-desktop-settings"),
                  )
                }
                icon={Icons.GEAR}
              >
          ...
          <Transition animation="slide-up" show={!!deviceName}>
            <DropdownLink href="/profiles" icon={Icons.USER}>
              {t("navigation.menu.switchProfile")}
            </DropdownLink>
            <DropdownLink
          ...
          <div className="my-4 flex justify-center items-center gap-4">
            {(conf() as any).TELEGRAM_LINK && (
              <CircleDropdownLink
                href={(conf() as any).TELEGRAM_LINK}
                icon={Icons.TELEGRAM}
              />
            )}
            <CircleDropdownLink
